...
async function main(){
    ...
    console.log(tx);    
    const nft0MID = "0xe5f897cd55b1481f0e3e18b76d6d83ea8109949c2fc63a46f30366b9130fcec0"
    let tx = await mosaique.getSnapshotStatus(nft0MID);
  
    console.log(tx.initialPrice);
        ...
        const aPersonalSignature = personalSign(accounts[1], aSigHash);
        // - Should reject request from account A due to role constraints
        const aSignerInstance = await mosaique.connect(accounts[1]);
        await expect(aSignerInstance.mintSnapshot(account1, aPersonalSignature, nft0MID, newTokenURI)).to.be.reverted;
        // - Should succeed
        tx = await mosaique.mintSnapshot(account1, aPersonalSignature, nft0MID, newTokenURI);
