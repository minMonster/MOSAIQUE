...
const bigNumberToString = (number) => {
  const bn = BigNumber.from(number.toString())
  return bn.toString()
}

const personalSign = async(signer, signatureMessageHash) => {
...
async function main() {
  ...
  const tx = await mosaique.createSnapshotSupply(account, personalSignature, nft0MID, initialPrice, totalSupply, priceCurve, birth, deadline)
  console.log(tx)
