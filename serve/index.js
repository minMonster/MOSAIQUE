...
// 编译合约
const source = 'pragma solidity ^0.4.0;contract Calc{  /*区块链存储*/  uint count;  /*执行会写入数据，所以需要`transaction`的方式执行。*/  function add(uint a, uint b) returns(uint){    count++;    return a + b;  }  /*执行不会写入数据，所以允许`call`的方式执行。*/  function getCount() constant returns (uint){    return count;  }}'
const calcCompiled = web3.eth.compile.solidity(source)
...
// 得到合约对象
const abiDefinition = calcCompiled['info']['abiDefinition']
const calcContract = web3.eth.contract(abiDefinition)

// 2. 部署合约

// 2.1 获取合约的代码，部署时传递的就是合约编译后的二进制码
const deployCode = calcCompiled['code']

// 2.2 部署者的地址，当前取默认账户的第一个地址。
const deployeAddr = web3.eth.accounts[0]

// 2.3 异步方式，部署合约
const myContractReturned = calcContract.new({
  data: deployCode,
  from: deployeAddr
